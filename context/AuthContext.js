...
export const AuthProvider = ({children}) => {
    ...
    const login = (email, password) => {
        setIsLoading(true)
        fetch(
            'https://erp.shambarecords.com/api/v1/login', {
                method:'POST',
                headers:{
        ...
        .then(data => {
            ...
            if(data.success === true){
                ...
                const jsonValue = JSON.stringify({
                    ...
                    id: data.data.user.role === 'farmer' ? data.data.user.id : data.data.user.cooperative.id,
    ...
    return(
        <AuthContext.Provider value = {{login, logout, isLoading, userToken, role, name, id, message}}>
            {children}
